...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**قام مطورين السورس بتحديث مـاتركس**\n**التـغييرات\n** {changelog}"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(
        event, "**⌔∮ يـتـم البـحـث عـن تـحديثـات سـورس مـاتركس انـتـظـر**"
    )
    ...
    if changelog == "" and not force_update:
        await event.edit(
            ...
            f"**قـنـاة سـورس مـاتركـس** : @VV399"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        await print_changelogs(event, ac_br, changelog)
        await event.delete()
        return await event.respond(f"⌔ : لتحديث سورس مـاتركـس ارسل : `.تحديث الان` ")
    ...
    if conf == "الان":
        await event.edit("**• جار تحـديـث سـورس مـاتركـس انـتـظـر قـليـلا 🔨**")
